<x-layouts.app title="Tambah Supplier Baru - Fixoria Sales">
    <div class="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-border shadow-sm">
            <div>
                <nav class="flex items-center gap-2 text-xs font-semibold text-secondary mb-2">
                    ...
                    <span class="text-on-surface font-bold">Tambah Supplier</span>
                </nav>
                <h2 class="font-display-lg text-xl sm:text-display-lg text-on-surface tracking-tight">Tambah Supplier Baru</h2>
                <p class="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">Lengkapi rincian identitas, kontak PIC, dan alamat mitra supplier baru.</p>
            </div>
            <div class="flex items-center gap-2 sm:gap-3 shrink-0 w-full sm:w-auto">
                ...
                <button type="submit" form="create-supplier-form" class="flex-1 sm:flex-none justify-center bg-primary hover:bg-primary/90 text-white px-4 sm:px-6 py-2.5 rounded-xl font-semibold flex items-center gap-2 shadow-lg shadow-primary/25 transition-all active:scale-95 text-xs sm:text-sm">
        ...
        <form id="create-supplier-form" action="{{ route('suppliers.store') }}" method="POST">
            @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <label class="flex items-center gap-3 h-11 px-4 rounded-xl border border-border bg-surface-container-lowest cursor-pointer hover:bg-canvas transition-colors">
                                    <input type="checkbox" id="input_is_active" name="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }} class="w-4 h-4 text-primary rounded border-border focus:ring-primary">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input class="w-full h-11 pl-10 pr-4 rounded-xl border border-border bg-surface-container-lowest text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all @error('email') border-error-text @enderror" id="input_email" name="email" value="{{ old('email') }}" placeholder="Contoh: kontak@sumbermakmur.com" type="email" />
                                </div>
                                @error('email')
                                    <span class="text-xs text-error-text mt-1 block">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="p-6 space-y-5">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="grow min-w-0">
                                    <h5 class="text-base font-bold text-on-surface truncate" id="preview_name">Nama Supplier Baru</h5>
                                    <p class="text-xs text-secondary truncate flex items-center gap-1 mt-0.5">
                                        <span class="material-symbols-outlined text-sm">person</span>
                                        <span id="preview_contact">PIC: -</span>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="border-t border-border pt-4 space-y-3">
                                <div class="flex items-center justify-between text-xs">
                                    ...
                                    <span class="font-mono font-semibold text-on-surface" id="preview_phone">-</span>
                                </div>
                                ...
                                <div class="flex items-center justify-between text-xs">
                                    ...
                                    <span class="font-semibold text-on-surface truncate max-w-[150px]" id="preview_email">-</span>
                                </div>
                                ...
                                <div class="border-t border-border/60 pt-3">
                                    ...
                                    <p class="text-xs text-on-surface line-clamp-2 leading-relaxed" id="preview_address">Belum diisi</p>
                                </div>
                            </div>
                            ...
                            <div class="border-t border-border pt-4 flex items-center justify-between">
                                ...
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100/70 text-emerald-800 border border-emerald-200" id="preview_status_container">
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-600" id="preview_status_dot"></span>
                                    <span id="preview_status_text">Aktif</span>
                                </span>
                            </div>
                        </div>
                    </div>
