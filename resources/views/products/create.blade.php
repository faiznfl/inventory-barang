<x-layouts.app title="Tambah Produk Baru - Fixoria Sales">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-border shadow-sm">
            <div>
                <nav class="flex items-center gap-2 text-xs font-semibold text-secondary mb-2">
                    ...
                    <span class="text-on-surface font-bold">Tambah Produk</span>
                </nav>
                <h2 class="font-display-lg text-xl sm:text-display-lg text-on-surface tracking-tight">Tambah Produk Baru</h2>
                <p class="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">Lengkapi rincian informasi, foto, serta pengaturan stok barang di bawah ini.</p>
            </div>
            <div class="flex items-center gap-2 sm:gap-3 shrink-0 w-full sm:w-auto">
                <a href="{{ route('products.index') }}" class="flex-1 sm:flex-none justify-center px-4 sm:px-5 py-2.5 bg-white border border-border text-on-surface rounded-xl font-semibold hover:bg-canvas transition-all flex items-center gap-2 text-xs sm:text-sm shadow-xs">
                    <span class="material-symbols-outlined text-sm">arrow_back</span>
                    Batal
                </a>
                <button type="submit" form="create-product-form" class="flex-1 sm:flex-none justify-center bg-primary hover:bg-primary/90 text-white px-4 sm:px-6 py-2.5 rounded-xl font-semibold flex items-center gap-2 shadow-lg shadow-primary/25 transition-all active:scale-95 text-xs sm:text-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select class="w-full h-10 px-3.5 rounded-xl border border-border bg-surface-container-lowest text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all @error('unit') border-error-text @enderror" id="input_satuan" name="unit" required>
                                            ...
                                            <option value="Pcs" {{ old('unit') == 'Pcs' ? 'selected' : '' }}>Pcs (Pcs)</option>
                            ...
                            <div class="md:col-span-5 flex flex-col">
                                ...
                                <div id="dropzone" class="flex-1 border-2 border-dashed border-border hover:border-primary rounded-xl p-5 text-center cursor-pointer transition-all bg-canvas/30 hover:bg-primary/5 flex flex-col items-center justify-center min-h-[210px] group relative">
                                    ...
                                    <div id="dropzone_prompt" class="space-y-2">
                                        ...
                                        <div>
                                            <p class="text-xs font-bold text-on-surface">Unggah Foto Produk</p>
                                    ...
                                    <div id="dropzone_preview" class="hidden w-full space-y-3">
                                        <div class="relative aspect-video w-full rounded-lg overflow-hidden border border-border bg-white shadow-xs">
                                            <img id="upload_thumb" src="" alt="Thumbnail" class="w-full h-full object-cover" />
                                        </div>
                                        <div class="flex items-center justify-between text-left px-1">
                                            <div class="truncate max-w-[130px]">
                                                <p id="upload_filename" class="text-xs font-bold text-on-surface truncate">nama-file.jpg</p>
                                                <p id="upload_filesize" class="text-[10px] text-on-surface-variant">0 KB</p>
                                            </div>
                            ...
                            <div class="md:col-span-12 pt-2">
                                ...
                                <textarea class="w-full rounded-xl border border-border p-3.5 bg-surface-container-lowest text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all @error('description') border-error-text @enderror" id="input_deskripsi" name="description" placeholder="Tuliskan detail deskripsi barang, spesifikasi material, garansi, atau catatan penting..." rows="3">{{ old('description') }}</textarea>
                                @error('description')
                                    <span class="text-xs text-error-text mt-1 block">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <section class="bg-white rounded-2xl p-6 md:p-8 border border-border shadow-xs space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input class="w-full h-10 px-3.5 rounded-xl border border-border bg-surface-container-lowest text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all @error('stock') border-error-text @enderror" id="input_stok" name="stock" value="{{ old('stock', 0) }}" placeholder="0" type="number" min="0" required />
                                @error('stock')
                                    <span class="text-xs text-error-text mt-1 block">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input class="w-full h-10 px-3.5 rounded-xl border border-border bg-surface-container-lowest text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all @error('min_stock') border-error-text @enderror" id="input_min_stok" name="min_stock" value="{{ old('min_stock', 10) }}" placeholder="10" type="number" min="0" />
                    ...
                    <div class="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <div class="w-full aspect-square rounded-xl bg-canvas mb-5 overflow-hidden flex items-center justify-center relative group border border-border/80 shadow-inner">
                                <img id="preview_image" class="w-full h-full object-cover transition-all duration-300" alt="Product Photograph" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBRXEXlJ6NHGD7AkGRt1t2dYxz11_J4H14mboxXFgRluSM0joql2WyLLF8LLUAhcGvmP5qJSHKv3-jHHj_UBTB4bZPcvaH3SLly_qNpDSvjQQPuLMlDqysrLHealKDCKCNAiVfGxMWW9KE00lm9uebh7cZliFoNoTXGn7bCNS6L7LvPdxTBZcM_zbkFHV6xUCrQyhY99n7jKGr7Tqtmim_dfQes-UaoUQ5KsGAtHLVfVJ59N7kep8mE"/>
                                <div class="absolute inset-0 bg-black/5 group-hover:bg-transparent transition-colors"></div>
                            </div>
                            
                            <div class="space-y-4">
                                <div>
                                    <span class="px-2.5 py-1 bg-primary/10 text-primary text-[10px] font-extrabold rounded-md uppercase tracking-wider inline-block" id="preview_kategori">Kategori</span>
                                    <h5 class="text-lg font-bold mt-2 text-on-surface line-clamp-2 leading-snug" id="preview_nama">Nama Produk Baru</h5>
                                </div>
                                ...
                                <div class="flex items-end justify-between border-t border-border pt-4">
                                    <div>
                                        ...
                                        <p class="text-xl font-extrabold text-primary" id="preview_harga">Rp 0</p>
                                    </div>
                    ...
                    <div class="bg-surface-container-low/70 rounded-2xl p-6 border border-border space-y-3">
                        ...
                        <ul class="space-y-3 text-xs text-on-surface-variant leading-relaxed">
                            <li class="flex items-start gap-2.5">
                                ...
                                <span><strong class="text-on-surface">Foto Ber Kualitas:</strong> Gunakan rasio 1:1 berpencahayaan baik agar Katalog Sales terlihat menarik.</span>
                            </li>
                            <li class="flex items-start gap-2.5">
                                <span class="w-1.5 h-1.5 rounded-full bg-primary shrink-0 mt-1.5"></span>
                                <span><strong class="text-on-surface">Format SKU:</strong> Gunakan pola unik seperti <code>[KAT]-[THN]-[ID]</code> untuk mempermudah scan gudang.</span>
                            </li>
                            <li class="flex items-start gap-2.5">
                                <span class="w-1.5 h-1.5 rounded-full bg-primary shrink-0 mt-1.5"></span>
                                <span><strong class="text-on-surface">Safety Stock:</strong> Disarankan minimal 15-20% dari rata-rata stok terpakai bulanan.</span>
                            </li>
                        </ul>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const defaultPlaceholderImage = "https://lh3.googleusercontent.com/aida-public/AB6AXuBRXEXlJ6NHGD7AkGRt1t2dYxz11_J4H14mboxXFgRluSM0joql2WyLLF8LLUAhcGvmP5qJSHKv3-jHHj_UBTB4bZPcvaH3SLly_qNpDSvjQQPuLMlDqysrLHealKDCKCNAiVfGxMWW9KE00lm9uebh7cZliFoNoTXGn7bCNS6L7LvPdxTBZcM_zbkFHV6xUCrQyhY99n7jKGr7Tqtmim_dfQes-UaoUQ5KsGAtHLVfVJ59N7kep8mE";
