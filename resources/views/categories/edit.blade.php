<x-layouts.app title="Edit Kategori - Fixoria Sales">
    <div class="p-4 sm:p-6 lg:p-8 space-y-6 max-w-4xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-border shadow-sm">
            <div>
                <nav class="flex items-center gap-2 text-xs font-semibold text-secondary mb-2">
                    ...
                    <span class="text-on-surface font-bold">Edit Kategori</span>
                </nav>
                <h2 class="font-display-lg text-xl sm:text-display-lg text-on-surface tracking-tight">Edit Kategori</h2>
                <p class="font-body-md text-xs sm:text-sm text-on-surface-variant mt-0.5">Perbarui nama dan deskripsi kategori produk di bawah ini.</p>
            </div>
            <div class="flex items-center gap-2 sm:gap-3 shrink-0 w-full sm:w-auto">
                <a href="{{ route('categories.index') }}" class="flex-1 sm:flex-none justify-center px-4 sm:px-5 py-2.5 bg-white border border-border text-on-surface rounded-xl font-semibold hover:bg-canvas transition-all flex items-center gap-2 text-xs sm:text-sm shadow-xs">
                    <span class="material-symbols-outlined text-sm">arrow_back</span>
                    Batal
                </a>
                <button type="submit" form="edit-category-form" class="flex-1 sm:flex-none justify-center bg-primary hover:bg-primary/90 text-white px-4 sm:px-6 py-2.5 rounded-xl font-semibold flex items-center gap-2 shadow-lg shadow-primary/25 transition-all active:scale-95 text-xs sm:text-sm">
        ...
        <div class="bg-white rounded-2xl p-6 md:p-8 border border-border shadow-xs">
            <form id="edit-category-form" action="{{ route('categories.update', $category) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
